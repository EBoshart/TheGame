...
public class GamePaneel extends JPanel implements KeyListener, ActionListener {
			...
			if (!(testCollission(rCube, rCharacter) && forward)) {
				...
				if (c.posX + rectsize < (getWidth() / 3)) {
					...
					for (int i = 0; i < numberOfBosses; i++) {
						...
						// rBoss[i].setBounds(boss[i].posX-10, getHeight() - 100
						// - boss[i].posY, 100+20, 100);
					}
				}
			}
		}
	...
	public void actionPerformed(ActionEvent e) {
		...
		for (int i = 0; i < numberOfBosses; i++) {
			...
			// rBoss[i].setBounds(boss[i].posX, getHeight() - 100 -
			// boss[i].posY, 100+20, 100);
		}
	...
	public void paintComponent(Graphics g) {
		...
		if (debug) {
			...
			debugTextPos += 15;
			g.drawString("Playtime: " + playTime, debugPos, debugTextPos);
			debugTextPos += 15;
		}
